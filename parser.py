...
def extract_shift_info(line, current_date):
    """
    Extracts all shift IDs from a line, cleans them, and classifies each by:
      - id: the cleaned shift code (e.g. '305', 'SA1', 'od7', 'OE')
      - type: one of 'Day', 'Evening', 'Night', or 'Other'
...
def parse_pdf(pdf_path):
    """
    Parses the flow sheet PDF at pdf_path, returning:
      - DataFrame of scheduled shifts (with Name, DateObj, Shift, Type, DayType, etc.)
      - List of any parsed swaps from the exceptions section
    """
    records = []
    swaps = []
    current_date = None
