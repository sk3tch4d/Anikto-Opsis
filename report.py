...
def process_report(pdf_paths): frames_with_swaps = [parse_pdf(p) for p in pdf_paths] frames = [f[0] for f in frames_with_swaps] swaps_all = sum([f[1] for f in frames_with_swaps], [])
...
def group_by_shift(df, target_date): from collections import defaultdict from datetime import datetime as dt

shifts = defaultdict(list)

for _, row in df.iterrows():
    start_time = dt.strptime(row["Start"], "%H:%M").time()
    start_datetime = dt.combine(row["DateObj"], start_time)
